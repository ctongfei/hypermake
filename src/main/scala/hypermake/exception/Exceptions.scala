...
case class AxisKeyNotFoundException(axis: Axis, key: String) extends Exception(s"Key “$key” not found in axis ${axis.name}.")
...
case class CyclicWorkflowException() extends Exception("Cyclic pipeline detected.")

case class JobFailedException(j: Job) extends Exception(s"Job ${j.colorfulString} failed.")

case class DataTransferFailedException(src: String, name: String) extends Exception(s"Failed to transfer file “$name” from “$src”.")
